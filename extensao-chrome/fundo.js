...
/* O cracha e POR ENDERECO.
   ...
   morava em um lugar unico. Agora ele pode estar no computador (localhost)
   ou na nuvem (Colab) -- e o cracha de um NAO vale no outro: sao servidores
   diferentes, com sessoes diferentes.
...
/* 09/09: esta lista só conhecia endereço de rede local. Com o Bigode no
   Colab, o endereço dele é um `trycloudflare.com` — e a aba do próprio
   Bigode voltou a ser tratada como site comum, trazendo de volta o eco que
   este bloco inteiro existe para evitar.

   Agora, além da rede local, comparamos com o endereço configurado: qualquer
   que seja o túnel de hoje, é ele que o painel está usando. */
function ehOProprioBigode(url) {
  const u = url || '';
  if (/^https?:\/\/(localhost|127\.0\.0\.1|192\.168\.\d+\.\d+)(:\d+)?(\/|$)/i.test(u)) return true;
  if (/^https:\/\/[a-z0-9-]+\.trycloudflare\.com(\/|$)/i.test(u)) return true;
  try {
...
async function bater() {
  ...
  /* Quando o Bigode está na nuvem, esta chamada atravessa a internet e um
     túnel. A espera longa de 25s do lado do servidor é segura (o Cloudflare
     corta perto de 100s), mas a rede pode cair no meio sem avisar — e aí a
     extensão ficaria pendurada para sempre.
  ...
  try {
    ...
    // Endereco de nuvem que para de responder quase sempre significa Colab
    // desconectado. Dizer isso poupa o Fred de procurar defeito na extensao.
    const naNuvem = !/^https?:\/\/(localhost|127\.|192\.168\.)/.test(base);
    motivo = naNuvem
      ? 'O Bigode da nuvem nao responde. O Colab desconectou? '
        + 'Rode o notebook de novo e cole o link novo nas opcoes.'
      : 'Nao consegui falar com o Bigode em ' + base
        + '. Confira o endereco e se ele esta aberto.';
