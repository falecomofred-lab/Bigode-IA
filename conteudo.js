...
function acaoCritica(acao, args, el) {
  ...
  const termos = ['comprar', 'finalizar', 'enviar', 'salvar', 'excluir', 'apagar', 'assinar', 'pagar', 'publicar', 'confirmar', 'submit'];
  return (acao === 'teclar' && String(args?.tecla || '').toLowerCase() === 'enter') ||
...
function aprovarNaPagina(acao, args, el) {
  return new Promise(resolve => {
    ...
    detalhe.textContent = 'O Bigode quer executar '+acao+' em “'+(rotulo(el) || 'elemento da página')+'”.';
...
chrome.runtime.onMessage.addListener((msg, _r, responder) => {
  ...
  const args = msg.args || {};
  const el = msg.acao === 'clicar' ? mapa[Number(args.numero)] : document.activeElement;

  (async () => {
    if (acaoCritica(msg.acao, args, el)) {
      const aprovado = await aprovarNaPagina(msg.acao, args, el);
      if (!aprovado) { responder({ resultado: 'Ação negada pelo usuário.' }); return; }
    }
