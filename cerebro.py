...
def config():
    ...
    # `setdefault` NAO COBRE CAMPO VAZIO                        (18/09)
    #
    #   `setdefault` so preenche quando a chave NAO EXISTE. Hoje, ao sair
    #   da nuvem pela tela, eu gravei "llm_url": "" -- chave presente, valor
    #   vazio. O default nunca entrou, o urllib recebeu string vazia e toda
    #   mensagem morria em 0,1s com:
    #
    #       Falha: unknown url type: ''
    #
    #   O motor local estava carregado e funcionando; o que faltava era o
    #   endereco dele. Uma linha de erro que nao diz nada sobre o que
    #   aconteceu, num ponto onde tudo parecia certo.
    #
    #   `or` em vez de `setdefault`: ausente e vazio passam a valer a mesma
    #   coisa, que e o que qualquer um esperaria ao ler o nome do campo.
    dados["llm_url"] = (str(dados.get("llm_url") or "").strip()
                        or "http://localhost:8082/v1/chat/completions")
    ...
    # A MODAL E O MESMO llama.cpp, EM OUTRO ENDERECO           (17/09)
    #
    #   O modal_motor.py sobe llama-server com --api-key e --jinja. Ou
    #   seja: mesma API compativel com a OpenAI que o motor local, mesmo
    #   `stream`, mesmos `tools`, mesmo `cache_prompt`.
    #
    #   Entao a Modal nao precisa de rota propria no chamar_modelo. Basta
    #   trocar o endereco e a chave aqui, e todo o caminho do motor local
    #   passa a valer: texto palavra por palavra, chamada de ferramenta
    #   remontada em pedacos, cache de prompt e medicao por etapa.
    #
    #   Havia um bloco separado para a Modal, que entregava o texto de uma
    #   vez e devolvia None no lugar da ferramenta. Foi apagado. Duas
    #   implementacoes da mesma coisa significam que os consertos de uma
    #   nunca chegam na outra -- e foi assim que a rota da Modal ficou sem
    #   ferramenta por tres dias sem ninguem notar.
    if str(dados.get("modelo_provedor")).lower() == "modal":
        m = dados.get("modal") or {}
        url = str(m.get("url") or "").strip()
        if url:
            dados["llm_url"] = url
            dados["llm_chave"] = str(m.get("token") or "").strip()
...
# Nome de campo que guarda segredo. Vale em qualquer nivel do config.
_SEGREDO = re.compile(
    r"token|chave|secret|senha|password|api_?key|client_secret|credential",
    re.I)


def _sem_segredos(valor):
    """Copia a estrutura trocando todo campo de segredo por um aviso.

    A REGRA E POR NOME, E NAO POR LISTA                        (20/09)

        Antes havia uma lista: github_token, codigo_acesso, chave_extensao
        e modal.token. Lista so protege o que alguem lembrou de escrever
        nela -- e dois segredos ja tinham escapado:

          . `llm_chave`, que o `config()` PREENCHE com o token da Modal
            quando o provedor e remoto. Tirar `modal.token` e deixar o
            `llm_chave` passar e tirar a chave da porta e esquecer a
            copia embaixo do tapete.

          . `huggingface.token`, que nem estava na lista.

        Por nome, o padrao vira seguro: campo novo com "token" ou "chave"
        no nome ja nasce protegido. Para vazar agora e preciso dar um nome
        que nao pareca segredo -- o que e um erro bem mais dificil de
        cometer sem perceber.

        A interface nao precisa do valor; precisa saber se EXISTE. Entao
        em vez de sumir com o campo, ele vira "(guardado)" -- assim a tela
        continua conseguindo mostrar "configurado" sem nunca ver o
        conteudo.
    """
    if isinstance(valor, dict):
        saida = {}
        for chave, item in valor.items():
            if _SEGREDO.search(str(chave)):
                saida[chave] = "(guardado)" if item else ""
            else:
                saida[chave] = _sem_segredos(item)
        return saida
    if isinstance(valor, list):
        return [_sem_segredos(x) for x in valor]
    return valor


def config_publica():
    """Configuração necessária para a interface, sem segredos."""
    dados = _sem_segredos(dict(config()))
    dados.pop("codigo_acesso", None)      # nao e "token" no nome, e e segredo
    sociais = dados.get("login_social")
    if isinstance(sociais, dict):
        dados["login_social"] = {
            provedor: {"configurado": bool(item.get("client_id"))}
...
def chamar_modelo(mensagens, ao_receber, com_ferramentas=True, ao_ferramenta=None,
    ...
    if cfg.get("modelo_provedor") == "huggingface" and huggingface_cliente.configurado():
        # A ROTA DO HUGGING FACE AGORA USA FERRAMENTA              (17/09)
        #
        #   Esta rota chamava `huggingface_cliente.chat`, que devolvia so
        #   texto, e terminava em `return texto_hf, None`. O None e o
        #   lugar da chamada de ferramenta -- entao por aqui o Bigode
        #   NUNCA usava ferramenta nenhuma. Sem ferramenta nao ha ler
        #   arquivo, nao ha buscar na web e nao ha mexer no navegador.
        #
        #   O conserto nao precisou de laco novo: `conversar` devolve a
        #   chamada no mesmo formato ("nome", {args}) que o motor local
        #   devolve, entao ela cai no laco que ja existia.
        #
        #   E `ao_receber` agora chega a cada pedaco, em vez de uma vez no
        #   fim: a tela volta a mostrar a resposta surgindo.
        inicio_hf = time.time()
        hf_cfg = cfg.get("huggingface") or {}
        marca = {"primeiro": None}

        def _pedaco(acumulado):
            if marca["primeiro"] is None:
                marca["primeiro"] = time.time()
            ao_receber(acumulado)

        texto_hf, chamada_hf = huggingface_cliente.conversar(
        ...
        if medidor is not None:
            primeiro = marca["primeiro"] or fim_hf
            medidor.anotar_modelo(
                prefill=primeiro - inicio_hf,
                decode=max(0.0, fim_hf - primeiro),
                pedacos=0,
                tokens_entrada=_tokens(json.dumps(mensagens, ensure_ascii=False)),
                tokens_saida=_tokens(texto_hf),
                obrigou_ferramenta=bool(obrigar_ferramenta),
            )
        return texto_hf, chamada_hf
    # A ROTA DA MODAL FOI APAGADA, E ISSO E UM GANHO           (17/09)
    #
    #   Havia um bloco inteiro aqui: chamava modal_cliente.gerar, entregava
    #   o texto de uma vez e terminava em `return texto_modal, None`. O
    #   None e o lugar da chamada de ferramenta -- entao pela Modal o
    #   Bigode NAO usava ferramenta nenhuma. Sem ferramenta ele nao le
    #   arquivo, nao busca na web e nao mexe no navegador: virava um chat.
    #
    #   O comentario que estava aqui dizia "o endpoint Modal atual nao
    #   implementa tool calling". Era verdade quando o endpoint era o
    #   esqueleto de teste. Deixou de ser: o modal_motor.py de hoje sobe
    #   llama.cpp com --jinja, que faz tool calling nativo.
    #
    #   E o llama.cpp na Modal e o MESMO servidor do motor local: mesma API
    #   compativel com a OpenAI, mesmo `stream`, mesmo `tools`, mesmo
    #   `cache_prompt`. Manter um cliente separado para ele era manter duas
    #   implementacoes da mesma coisa -- e uma delas, a da Modal, ia ficando
    #   para tras a cada conserto feito na outra.
    #
    #   Agora o `config()` aponta llm_url e llm_chave para o endereco da
    #   Modal quando o provedor e "modal", e o codigo segue daqui para
    #   baixo, igual ao local. A rota da Modal ganhou de uma vez:
    #   ferramentas, texto palavra por palavra, cache de prompt e medicao
    #   por etapa -- sem uma linha nova.
    payload = {
        "messages": mensagens,
...
def extrair_chamada(texto):
    ...
    # 2) A ETIQUETA E O NOME DA FERRAMENTA                     (18/09)
    #
    #    O protocolo pede <ferramenta>{"nome": ..., "args": {...}}</ferramenta>.
    #    O DeepSeek-Coder-V2 escreve assim:
    #
    #        <listar_pasta>
    #        {"caminho": "G:\\Meu Drive\\projetos"}
    #        </listar_pasta>
    #
    #    Ele pos o NOME no lugar da etiqueta e mandou so os argumentos. Nao e
    #    besteira do modelo: e o formato que varios modelos usam, e a
    #    intencao esta clarissima. Mas o parser nao casava, nada executava,
    #    e o XML cru aparecia na resposta -- foi o que o Fred viu hoje,
    #    sete vezes seguidas, com o guarda de laco cortando no fim.
    #
    #    Aceitar isto custa uma expressao regular. Exigir que o modelo
    #    escreva do MEU jeito custa a ferramenta inteira.
    #
    #    A trava contra falso positivo e o `in ferramentas.ativas()`: so
    #    vira chamada se a etiqueta for o nome de uma ferramenta que existe
    #    e esta ligada. `<div>` e `<p>` num codigo HTML que ele escreveu
    #    continuam sendo texto.
    for achado in re.finditer(r"<([a-z][a-z0-9_]{2,40})>\s*(\{.*?\})\s*</\1>",
                              texto, re.S | re.I):
        etiqueta = achado.group(1).lower()
        if etiqueta in ("ferramenta", "tool_call", "function_call"):
            continue          # ja tratados acima
        if etiqueta not in ferramentas.ativas():
            continue
        try:
            args = json.loads(achado.group(2))
            if isinstance(args, dict):
                # Alguns embrulham em "args"/"arguments"; outros nao.
                interno = args.get("args") or args.get("arguments")
                return etiqueta, (interno if isinstance(interno, dict) else args)
        except Exception:
            continue

    # 3) JSON solto no meio da resposta, sem marcador nenhum. So vale a pena
...
def consulta_web_direta(pergunta):
    ...
    # ═══ O VERBO PRECISA COMECAR A FRASE ═════════════════ (18/09) ═══
    #
    #   O Fred escreveu "consegue buscar algumas?" -- continuando uma
    #   conversa sobre skills de UI/UX. O atalho viu "buscar", pegou tudo
    #   o que vinha depois e pesquisou a palavra ALGUMAS. Voltaram sete
    #   links de dicionario explicando o pronome indefinido.
    #
    #   O atalho existe para "pesquise sobre cannabis" -- uma ORDEM, com o
    #   assunto logo atras. "consegue buscar algumas?" e outra coisa: e uma
    #   pergunta cujo objeto esta na mensagem ANTERIOR. Sem o historico, o
    #   que vem depois do verbo nao e consulta nenhuma.
    #
    #   O `^` resolve o caso real sem inventar heuristica: ordem de
    #   pesquisa comeca com o verbo. No meio da frase, deixa o modelo
    #   decidir -- ele tem o historico, o atalho nao.
    achado = re.match(
        r"^(?:me\s+)?(?:pesquise|pesquisar|pesquisa|busque|buscar|procure|"
        r"procurar)\b"
    ...
    # Palavra solta que so faz sentido com o que veio antes. Pesquisar
    # "algumas" ou "isso" nunca vai ajudar; melhor o modelo perguntar.
    VAGAS = {"algumas", "alguns", "algum", "alguma", "isso", "isto", "aquilo",
             "mais", "outras", "outros", "outro", "outra", "esses", "essas",
             "aquele", "aqueles", "elas", "eles", "ele", "ela", "la", "ai"}
    if consulta.lower() in VAGAS or len(consulta) < 3:
        return ""
    return consulta


# ══════════════════════════════════════════════════════════════════════
# PEDIDO DE DESENHO VAI DIRETO, SEM PASSAR PELO MODELO       (18/09/2026)
# ══════════════════════════════════════════════════════════════════════
#
#   "crie uma imagem de um gato laranja usando a pipi ia"
#
#   Duas tentativas, duas vezes errado. O log conta:
#
#       11:55  web_buscar  "como criar uma imagem de um cafe ao amanhecer..."
#       11:59  navegador_ver  25s
#
#   Nenhuma chamada a gerar_imagem. Na segunda ele chegou a citar a Pipi
#   pelo nome e mesmo assim foi pesquisar tutorial no Google e olhar a
#   tela. 63 segundos para nao desenhar nada.
#
#   NAO E FALTA DE FERRAMENTA. `crie uma imagem` e gatilho do grupo
#   "desenho", e o `gerar_imagem` estava no catalogo. O problema e que o
#   catalogo tem 34 ferramentas -- o filtro por pergunta esta DESLIGADO de
#   proposito, por medicao, para nao quebrar o cache de prompt -- e um 7B
#   escolhendo entre 34 opcoes erra.
#
#   Este arquivo ja resolveu exatamente isso uma vez, para a pesquisa: o
#   `consulta_web_direta` pega "pesquise sobre X" e roda a busca sem gastar
#   uma rodada do modelo perguntando palavra-chave. O comentario de la diz
#   "nao deixa o modelo fabricar uma resposta de preenchimento".
#
#   "crie uma imagem de X" e tao explicito quanto "pesquise sobre X". Nao
#   ha o que deliberar: a frase JA diz a ferramenta e JA diz o argumento.
#   Mandar isso para um modelo pequeno decidir e criar uma chance de erro
#   onde nao havia duvida.
#
#   OS GATILHOS AQUI SAO MAIS ESTREITOS QUE OS DO CATALOGO, de proposito.
#   La, incluir uma ferramenta a mais custa tokens. Aqui, um falso positivo
#   EXECUTA -- gasta placa e entrega uma imagem que ninguem pediu. Por isso
#   ficaram de fora "foto de" e "criativo", que aparecem em frase comum
#   ("me manda a foto de ontem"). So imperativo sem ambiguidade entra.
DESENHO_DIRETO = re.compile(
    r"\b(?:crie|cria|criar|gere|gera|gerar|desenhe|desenha|desenhar|"
    r"faca|faça|monte|monta)\s+"
    r"(?:uma?\s+)?(?:nova\s+)?"
    r"(?:imagem|ilustracao|ilustração|desenho|arte|banner|thumbnail|"
    r"poster|pôster|wallpaper|papel\s+de\s+parede)\b"
    r"\s*(?:de|do|da|dos|das|com|sobre|para|:)?\s*(.*)$",
    re.I | re.S)

# O formato dito na propria frase. "formato story" e "para story" sao a
# mesma coisa para quem fala; a Pipi so entende a palavra seca.
FORMATOS_DITOS = (
    ("story", "story"), ("stories", "story"), ("9:16", "story"),
    ("retrato", "retrato"), ("vertical", "retrato"), ("2:3", "retrato"),
    ("paisagem", "paisagem"), ("horizontal", "paisagem"), ("3:2", "paisagem"),
    ("capa", "capa"), ("banner", "capa"), ("16:9", "capa"),
    ("quadrado", "quadrado"), ("1:1", "quadrado"),
)


def pedido_de_desenho(pergunta):
    """(descricao, formato) se a frase e um pedido claro de imagem."""
    texto = " ".join(str(pergunta or "").split())
    achado = DESENHO_DIRETO.search(texto)
    if not achado:
        return "", ""

    descricao = achado.group(1).strip(" .,;:!?-")
    baixo = texto.lower()

    formato = ""
    for palavra, nome in FORMATOS_DITOS:
        if palavra in baixo:
            formato = nome
            break

    # "usando a pipi ia", "na pipi", "com o flux" -- o usuario dizendo por
    # ONDE, nao O QUE. Isso nao pode entrar no prompt da imagem, senao o
    # FLUX tenta desenhar as palavras.
    descricao = re.sub(
        r"\b(?:usando|use|com|na|no|pela|pelo|atraves da?|através da?)\s+"
        r"(?:a\s+|o\s+)?(?:pipi(?:\s*ia)?|flux|modal|sd|stable\s*diffusion)\b",
        "", descricao, flags=re.I).strip(" .,;:-")
    # E a palavra de formato, que ja virou parametro.
    if formato:
        descricao = re.sub(
            r"\b(?:no\s+|em\s+|formato\s+|para\s+)?%s\b" % re.escape(formato),
            "", descricao, flags=re.I).strip(" .,;:-")
        descricao = re.sub(r"\bformato\b\s*$", "", descricao,
                           flags=re.I).strip(" .,;:-")

    return (descricao, formato) if len(descricao) >= 3 else ("", "")
...
# ══════════════════════════════════════════════════════════════════════
# O MESMO MODELO, NOS DOIS LUGARES                          (18/09/2026)
# ══════════════════════════════════════════════════════════════════════
#
#   A tela listava so os .gguf do disco, e clicar significava sempre "subir
#   na minha CPU". Mas o mesmo Qwen 7B e o mesmo DeepSeek estao publicados
#   na Modal, onde rodam dezenas de vezes mais rapido -- e nao havia como
#   escolher isso de dentro da tela.
#
#   Agora cada cartao sabe se existe uma copia na placa, e o clique
#   pergunta ONDE, nao SE.
#
#   O AMARRADOR E O NOME DO ARQUIVO .gguf
#       O `modal_motor.py` guarda, em cada perfil, o nome exato do arquivo
#       que aquele app carregou. E o mesmo nome do arquivo local. Entao
#       ligar os dois nao pede tabela nova nem configuracao: e a mesma
#       chave que o `listar()` do modelos.py ja usa para deduplicar.
#
#   POR QUE NAO PUBLICAR SOZINHO AO CLICAR
#       Um deploy baixa 5 a 10 GB e passa a cobrar Volume por mes. Isso e
#       uma decisao de dinheiro, e decisao de dinheiro nao se toma por
#       clique acidental. O cartao mostra o comando; quem roda e o Fred.
def _perfis_da_placa():
    """{nome_do_arquivo_gguf: {perfil, app, publicado}} — o que ha na Modal.

    LIA DO `modal_motor`, E ISSO ESCONDIA OS BOTOES            (18/09)

        A tabela morava dentro do modal_motor.py, que comeca com
        `import modal`. Esta copia do Bigode roda no Python portatil, onde
        a biblioteca `modal` nao esta instalada -- o import falhava, o
        `except` devolvia {}, e a tela mostrava todos os cartoes SEM o
        botao da placa. Sem erro, sem aviso: como se a Modal nao existisse.

        Agora a tabela mora no `perfis_modal.py`, que nao importa nada.
        Dado e dado; biblioteca fica com quem precisa dela.
    """
    try:
        import perfis_modal
        catalogo = dict(perfis_modal.PERFIS)
    except Exception as erro:
        # Se ate isto falhar, o motivo vai para o log. `except: pass` foi
        # o que transformou "quebrado" em "nao existe" da primeira vez.
        anotar_erro("[placa] nao consegui ler perfis_modal.py", erro)
        return {}

    guardados = ((config().get("modal") or {}).get("perfis") or {})
    saida = {}
    for perfil, dados in catalogo.items():
        arquivo = str(dados.get("arquivo") or "").lower()
        if not arquivo:
            continue
        saida[arquivo] = {
            "perfil": perfil,
            "app": dados.get("app", ""),
            "nota": dados.get("nota", ""),
            # "publicado" = ja tem endereco guardado aqui. Nao consulta a
            # Modal: isto roda a cada abertura de tela, e uma chamada de
            # rede por cartao deixaria o painel lento por nada.
            "publicado": bool(str((guardados.get(perfil) or {}).get("url")
                                  or "").strip()),
        }
    return saida
...
def estado_da_placa():
    m = config().get("modal") or {}
    return {
        "provedor": str(config().get("modelo_provedor") or "local").lower(),
        "perfil": str(m.get("perfil") or ""),
        "tem_alguma": bool((m.get("perfis") or {})),
    }


def modelos_com_placa():
    """A lista de sempre, com o que existe na Modal marcado em cada item."""
    placa = _perfis_da_placa()
    atual = estado_da_placa()
    lista = listar_modelos()
    for item in lista:
        chave = str(item.get("arquivo") or "").lower()
        # `arquivo` vem sem extensao (stem); os perfis guardam com .gguf.
        achado = placa.get(chave + ".gguf") or placa.get(chave)
        if not achado:
            continue
        item["placa"] = achado
        item["na_placa_agora"] = (atual["provedor"] == "modal"
                                  and atual["perfil"] == achado["perfil"])
    return lista


def ligar_na_placa(perfil):
    """Aponta o Bigode para o app da Modal daquele modelo.

    Nao publica nada e nao baixa nada: so troca para onde as perguntas vao.
    Se o perfil ainda nao foi publicado, devolve o comando em vez de tentar
    adivinhar um endereco.

    Quem faz o trabalho e o `motor_perfil.py` -- o mesmo que o BIGODE.bat
    chama. Uma logica so para a tela e para a partida: se um dia o formato
    do config mudar, muda num lugar.
    """
    perfil = (perfil or "").strip().lower()
    if perfil not in ("conversa", "codigo"):
        return {"ok": False, "msg": "perfil desconhecido: %r" % perfil}

    guardados = ((config().get("modal") or {}).get("perfis") or {})
    if not str((guardados.get(perfil) or {}).get("url") or "").strip():
        return {"ok": False, "nao_publicado": True, "perfil": perfil,
                "msg": ("Este modelo ainda nao esta publicado na sua conta "
                        "da Modal. No PowerShell, na pasta do Cerebro:\n\n"
                        "    set BIGODE_PERFIL=%s\n"
                        "    python -m modal deploy modal_motor.py\n"
                        "    python usar_modal_motor.py\n\n"
                        "O deploy baixa alguns GB e leva uns minutos. "
                        "Depois disso, este botao passa a funcionar."
                        % perfil)}

    import subprocess
    try:
        r = subprocess.run(
            [sys.executable, str(BASE / "motor_perfil.py"), perfil],
            capture_output=True, text=True, encoding="utf-8",
            errors="ignore", timeout=60)
    except Exception as erro:
        return {"ok": False, "msg": "nao consegui trocar: %s" % erro}

    if r.returncode != 0:
        return {"ok": False,
                "msg": (r.stdout or r.stderr or "").strip()[:300] or
                       "o motor_perfil.py recusou a troca"}

    # O motor local vira peso morto: 6 GB de RAM e doze threads segurando
    # um modelo que nao vai mais atender pedido nenhum.
    try:
        PROVEDOR.descarregar()
    except Exception:
        pass

    anotar_erro("[motor] indo para a placa, perfil=%s, a pedido da tela"
                % perfil)
    return {"ok": True, "msg": "ligando na placa (%s)" % perfil}


def trocar_modelo(caminho, voltar_para_local=False):
    """Liga um modelo local. Com `voltar_para_local`, sai da nuvem antes.

    POR QUE ESTE SEGUNDO PARAMETRO EXISTE                     (18/09)

        Ontem pus uma trava no `carregar`: com o provedor em Modal ou
        Hugging Face, ele se recusa a subir motor local -- porque isso
        gastava 6,3 GB de RAM e doze threads com um modelo que nao ia
        atender pedido nenhum.

        A trava esta certa. O que estava errado era o caminho de volta:
        a recusa mandava o Fred abrir um terminal e rodar
        `usar_modal_motor.py --remover`. Ele estava clicando num botao
        LIGAR, na tela, olhando para o modelo que queria. Mandar alguem
        para a linha de comando para desfazer um clique e transformar uma
        decisao de dois segundos num procedimento.

        Agora a propria tela pergunta e resolve. A trava continua
        existindo -- ninguem sobe motor local sem querer --, mas quem
        quer, consegue de onde esta.
    """
    if voltar_para_local and PROVEDOR.remoto():
        antes = str(config().get("modelo_provedor") or "")
        # `llm_url` e `llm_chave` foram sobrescritos pelo endereco da nuvem
        # em `config()`. Zerar so o provedor deixaria o motor local
        # apontando para a Modal -- o pior dos dois mundos.
        #
        # 18/09, minutos depois: eu tinha escrito `"llm_url": ""` aqui, e
        # isso quebrou TODA conversa com "unknown url type: ''". O
        # `config()` preenchia o padrao com `setdefault`, que so age quando
        # a chave falta -- e eu tinha deixado a chave presente e vazia.
        #
        # O `config()` ja foi corrigido para tratar vazio como ausente.
        # Mesmo assim, o certo AQUI e escrever o endereco local de verdade,
        # em vez de apagar e torcer para alguem preencher depois. Quem
        # desfaz uma troca devolve o estado anterior; nao deixa um buraco.
        salvar_config({"modelo_provedor": "local",
                       "llm_url": "http://localhost:8082/v1/chat/completions",
                       "llm_chave": ""})
        anotar_erro("[motor] saindo de %s para local, a pedido da tela"
                    % (antes or "nuvem"))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # A CAPTACAO DE VOZ, PELO MESMO MOTIVO DO CSS              (17/09)
        #
        #   O voz.js e um arquivo so, usado pelo Bigode e pela Pipi. Ele
        #   tem a parte dificil: medir o volume, parar sozinho quando voce
        #   cala, montar o WAV de 16 kHz que o Whisper espera.
        #
        #   Estava escrito duas vezes, quase igual, nos dois index.html --
        #   e so um dos dois ia ganhar o proximo conserto.
        #
        #   Livre de login como o CSS: a tela carrega o script antes de
        #   existir sessao, e um 401 aqui apareceria como "Voz is not
        #   defined" no console, que nao explica nada.
        if rota == "/voz.js":
            arq = BASE / "web" / "voz.js"
            if arq.is_file():
                return self._enviar(200,
                                    ...
                                    arq.read_bytes())
            return self._enviar(404, "text/plain", b"voz.js nao encontrado")
        ...
        rotas = {
            ...
            "/api/status": lambda: self._json({
                **PROVEDOR.status(),
                ...
                # O PAINEL DA TELA PRECISA SABER ONDE O MOTOR RODA  (17/09)
                #
                #   A tela mostrava o NOME do modelo e nada sobre onde ele
                #   estava: local, Hugging Face ou Modal. Sao tres coisas
                #   com custo, velocidade e limite bem diferentes -- e ler
                #   "Qwen2.5-Coder" nao diz qual delas esta valendo.
                "provedor": config().get("modelo_provedor", "local"),
                "ferramentas": len(ferramentas.CATALOGO),
            }),
            "/api/modelos": lambda: self._json({
                "modelos": modelos_com_placa(),
                "placa": estado_da_placa(),
                "mcp": "%s %s" % (
    ...
    def do_POST(self):
        ...
        if rota == "/api/modelo":
            return self._enviar(200, *self._json(trocar_modelo(
                dados.get("caminho", ""),
                voltar_para_local=bool(dados.get("voltar_para_local")))))
        if rota == "/api/modelo/placa":
            return self._enviar(200, *self._json(
                ligar_na_placa(str(dados.get("perfil") or ""))))
    ...
    def _chat_interno(self, dados):
        ...
        # ── DESENHO TAMBEM VAI DIRETO ───────────────────────────── (18/09)
        #
        # Mesmo raciocinio da pesquisa acima, pelo mesmo motivo. "crie uma
        # imagem de X" nao tem o que deliberar: a frase diz a ferramenta e
        # diz o argumento. Ver o cabecalho de `pedido_de_desenho`.
        desc_imagem, fmt_imagem = pedido_de_desenho(pergunta)
        if desc_imagem and "gerar_imagem" in ferramentas.ativas():
            evento({"tipo": "acao", "id": "desenho-direto", "estado": "rodando",
                    "titulo": "Pedindo a imagem para a Pipi",
                    "resumo": desc_imagem[:100]})
            inicio_img = time.time()
            saida_img = ferramentas.executar(
                "gerar_imagem", {"descricao": desc_imagem,
                                 "formato": fmt_imagem})
            gasto_img = time.time() - inicio_img
            medidor.anotar_ferramenta("gerar_imagem", gasto_img,
                                      {"descricao": desc_imagem})
            try:
                anotar_erro("[desenho direto] %.1fs  formato=%r  desc=%r\n"
                            "-> %s" % (gasto_img, fmt_imagem, desc_imagem[:120],
                                       (saida_img or "")[:300]))
            except Exception:
                pass

            deu_certo = "http" in (saida_img or "")
            evento({"tipo": "acao", "id": "desenho-direto",
                    "estado": "ok" if deu_certo else "erro",
                    "titulo": "A Pipi desenhou" if deu_certo
                              else "Não consegui desenhar",
                    "resumo": desc_imagem[:100]})
            evento({"tipo": "texto", "texto": saida_img})
            # A imagem E a prova. Sem isto o carimbo "NAO CONFERI" cairia
            # em cima de uma imagem que acabou de ser criada -- o mesmo
            # defeito que a leitura de pagina tinha ate hoje de manha.
            if deu_certo:
                medidor.arquivos_escritos.append("imagem da Pipi")
            evento({"tipo": "medicao", **medidor.resumo(deu_certo)})
            medidor.gravar(deu_certo)
            evento({"tipo": "fim"})
            return
        ...
        if (ferramentas.ligada_navegador() and extensao_conhecida()
            ...
            # ═══ PAGINA VAZIA NAO E PAGINA LIDA ═══════════════ (18/09) ═══
            #
            # Este bloco dizia "Olhei a sua tela" e mandava ao modelo:
            #
            #     RESULTADO de navegador_ver (...):
            #     <nada>
            #     [Voce JA TEM a pagina acima, com os numeros de cada
            #      elemento. NAO chame navegador_ver de novo.]
            #
            # ...tambem quando `pagina` voltava vazia -- extensao dormindo,
            # sem recarregar depois de uma mudanca no manifest, ou os 60s
            # estourados. O modelo era informado de que TINHA uma pagina que
            # nao existia, e proibido de buscar.
            #
            # A resposta que saia disso, tres vezes em dois dias:
            #
            #     "Entendido! Vou ajudar com base na pagina que voce
            #      compartilhou. Por favor, me diga qual acao voce deseja."
            #
            # Passei esses tres episodios achando que era limitacao do 7B.
            # Nao era: um modelo obediente, avisado de que tem a pagina e
            # proibido de buscar, so pode fingir que a leu. Ele estava
            # fazendo exatamente o que o prompt mandava.
            #
            # E a narracao mentia junto: "Olhei a sua tela" com estado "ok"
            # sobre leitura nenhuma. A tela que deveria denunciar a falha
            # confirmava o sucesso -- foi por isso que demorei tanto.
            # ═══ E POR QUE `bool(pagina)` NAO BASTAVA ═════════ (18/09) ═══
            #
            # Corrigi o caso da pagina vazia e o defeito continuou igual.
            # Porque nao era vazia: quando so a aba do proprio Bigode esta
            # aberta, a extensao devolve uma FRASE --
            #
            #     "A unica aba aberta no Chrome e a tela do proprio Bigode.
            #      Nao ha site nenhum para eu ler. Abra uma aba com o site..."
            #
            # -- que e um recado perfeito para uma pessoa e um veneno para o
            # modelo, porque chega rotulado como "RESULTADO de navegador_ver
            # (a pagina que o Fred esta vendo NESTE MOMENTO)" e seguido de
            # "voce JA TEM a pagina acima". O modelo recebe um aviso de
            # fracasso embrulhado como sucesso.
            #
            # O problema de fundo: conteudo e erro voltam pelo MESMO canal,
            # como texto solto, sem nada que os distinga. Eu tinha testado
            # "veio alguma coisa?" quando a pergunta certa era "veio uma
            # PAGINA?". Presenca de resposta nao e presenca de resultado.
            #
            # O conteudo.js monta toda leitura de verdade comecando com
            # `PÁGINA: <titulo>`. Entao esse cabecalho e a assinatura: com
            # ele, e pagina; sem ele, e recado -- e recado vai para o Fred,
            # com todas as letras, em vez de virar contexto falso.
            bruto = (pagina or "").strip()
            leu = bruto.startswith("PÁGINA:") or bruto.startswith("ENDEREÇO:")

            # ═══ REGISTRO DO QUE A EXTENSAO DEVOLVEU ══════════ (18/09) ═══
            #
            # Tres consertos seguidos neste mesmo ponto, os tres por palpite,
            # os tres errados. Em cada rodada eu deduzia o conteudo em vez de
            # olhar para ele -- e o unico lugar onde ele aparecia era um
            # "ver detalhes" que exige clique.
            #
            # Entao ele passa a ser gravado. Nao e erro: e a evidencia que
            # faltava, no arquivo que o painel de diagnostico ja mostra.
            # Instrumentar sai mais barato do que a quarta hipotese.
            # O `ramo` responde sozinho a pergunta que eu teria de fazer a
            # cada rodada: "voce reiniciou o Bigode depois da minha
            # mudanca?". Se ele aparecer no log, o codigo novo esta rodando.
            # Se nao aparecer, esta rodando o antigo -- e eu paro de
            # procurar defeito numa correcao que nunca entrou em vigor.
            try:
                anotar_erro("[navegador_ver] leu=%s  %d caracteres  "
                            "ramo=%s\n--- primeiros 400 ---\n%s\n--- fim ---"
                            % (leu, len(bruto),
                               "ACAO" if _verbo_de_acao(pergunta) else "PERGUNTA",
                               bruto[:400] or "(vazio)"))
            except Exception:
                pass
            ...
            if leu:
                ...
                # entregado a ele. Mesmo assim ele chamou navegador_ver DE
                # NOVO -- mais 90 segundos de espera para receber o que ja
                # estava na mao -- e depois respondeu em texto, sem teclar.
                #
                # Modelo pequeno nao infere "ja tenho isso". Tem que ser dito.
                # ═══ PERGUNTA E ACAO PRECISAM DE INSTRUCOES DIFERENTES ═══
                #                                                (18/09)
                # A instrucao daqui era uma so, e era toda sobre AGIR:
                #
                #     "[Voce JA TEM a pagina acima, com os numeros de cada
                #      elemento. NAO chame navegador_ver de novo. Se o
                #      pedido do Fred e uma acao, execute-a agora:
                #      navegador_clicar, navegador_escrever ou
                #      navegador_teclar, usando os numeros.]"
                #
                # O Fred perguntou "o que voce ve na minha tela?". Isso nao
                # e acao. O modelo recebeu a pagina inteira -- titulo,
                # endereco e 4.000 caracteres de texto -- e um unico
                # comando, que falava so de clicar e escrever. Respondeu:
                #
                #     "Vou ajudar com base na pagina que voce compartilhou.
                #      Por favor, me diga qual acao voce deseja realizar."
                #
                # Ele estava DEVOLVENDO a instrucao. Pedi acao, ele pediu
                # acao de volta. Nao faltou pagina e nao faltou capacidade:
                # faltou dizer o que fazer quando a pergunta e pergunta.
                #
                # Passei quatro rodadas culpando o 7B por nao responder uma
                # pergunta que eu nunca mandei ele responder.
                if _verbo_de_acao(pergunta):
                    ordem = ("[Voce JA TEM a pagina acima, com os numeros de "
                             "cada elemento. NAO chame navegador_ver de novo. "
                             "O pedido do Fred e uma ACAO: execute-a agora "
                             "com navegador_clicar, navegador_escrever ou "
                             "navegador_teclar, usando os numeros da lista.]")
                else:
                    ordem = ("[Voce JA TEM a pagina acima e NAO deve chamar "
                             "navegador_ver de novo. O Fred fez uma PERGUNTA "
                             "sobre esta tela -- responda a pergunta usando "
                             "PAGINA, ENDERECO e TEXTO VISIVEL acima. Diga o "
                             "nome do site e o que ha nele. NAO pergunte qual "
                             "acao ele deseja: ele nao pediu acao nenhuma.]")
                mensagens.append({"role": "user", "content":
                                  "RESULTADO de navegador_ver (a pagina que o "
                                  "Fred esta vendo NESTE MOMENTO):\n"
                                  + pagina[:3500] + "\n\n" + ordem})
                usou_ferramenta = True
                lidos.append("a página aberta no Chrome")

                # ═══ A PAGINA LIDA CONTA COMO FONTE ═══════════════ (18/09)
                #
                # O Fred perguntou o que havia na tela. O Bigode leu a
                # pagina, descreveu certo -- e levou "ATENCAO: NAO CONFERI,
                # escreveu de memoria, trate como rascunho".
                #
                # Porque o `navegador_ver` entra por fora do laco de
                # ferramentas: ele e injetado direto, e o `anotar_ferramenta`
                # nunca roda. O medidor terminava com `arquivos_lidos` vazio,
                # `tem_evidencia` dava False, e o carimbo saia.
                #
                # Carimbo em resposta certa e pior do que carimbo nenhum --
                # esta exata frase ja esta escrita neste arquivo, no conserto
                # de 24/08, sobre listar pastas. Mesmo defeito, outra porta.
                # Quem le prova sem registrar a prova acaba desmentindo a si
                # mesmo.
                try:
                    medidor.arquivos_lidos.append(
                        "página aberta: " + (de_onde or "Chrome"))
                except Exception:
                    pass

                # Se o pedido era para AGIR, a proxima fala dele tem que ser
                # uma ferramenta, nao uma frase. Ele ja respondeu "Aguarde…
                # abrindo a pagina de pesquisa" sem abrir nada -- narrar uma
                # acao no lugar de executa-la e o pior desfecho aqui, porque
                # parece que funcionou.
                if _verbo_de_acao(pergunta):
                    obrigar = True
            else:
                # A extensao explica melhor do que eu o que houve: ela sabe
                # se so havia a aba do Bigode, se era uma chrome:// ou se
                # nao havia aba nenhuma. Repasso o motivo dela e nao invento
                # um meu.
                motivo = bruto[:300] or ("a extensão do Chrome não respondeu "
                                         "(pode estar dormindo, ou precisa "
                                         "ser recarregada)")
                evento({"tipo": "acao", "id": "olhar", "estado": "erro",
                        "titulo": "Não consegui ler a sua tela",
                        "detalhe": motivo[:120]})
                # Dizer a VERDADE ao modelo. Sem pagina, o certo e ele
                # avisar que nao viu -- nao inventar que viu.
                mensagens.append({"role": "user", "content":
                                  "AVISO DO SISTEMA: tentei ler a pagina "
                                  "aberta no Chrome e NAO consegui. Motivo:\n"
                                  + motivo +
                                  "\n\nVoce NAO tem a pagina. Nao finja que "
                                  "leu, nao descreva conteudo nenhum dela e "
                                  "nao peca 'qual acao deseja realizar'. "
                                  "Explique ao Fred, em uma frase e com suas "
                                  "palavras, o motivo acima. Depois responda "
                                  "ao que ele perguntou, com o que voce sabe."})
        ...
        for passo in range(int(cfg["max_passos"])):
            ...
            def ao_receber(acumulado):
                ...
                # 18/09: a janela era de 400 caracteres. O laco de hoje
                # ("P.S.2, P.S.4, P.S.5, P.S.7...") gastou 60 segundos e so
                # foi cortado na QUARTA repeticao -- o limite ja era 3, mas
                # entre duas conferencias cabiam duas repeticoes inteiras.
                # Conferir de 200 em 200 corta na terceira e devolve meio
                # minuto. Custa uma contagem de frases a mais por resposta.
                if len(filtrado) - estado.get("visto_em", 0) > 200:
                    ...
                    # O POS-ESCRITO NUMERADO E UM LACO DISFARCADO
                    #
                    # "P.S.2:", "P.S.4:", "P.S.5:", "P.S.7:" -- cada um com
                    # texto um pouco diferente, entao a contagem de frases
                    # identicas acima nao pega. Mas ninguem escreve tres
                    # pos-escritos numerados de proposito: e o modelo tentando
                    # encerrar e nao achando a saida.
                    #
                    # O numero pulado (2, 4, 5, 7) e o sinal mais claro: ele
                    # nem esta contando, so repetindo a forma.
                    if len(re.findall(r"\bP\.?S\.?\s*\d", filtrado, re.I)) >= 3:
                        anotar_erro("texto em laco: pos-escritos numerados "
                                    "em sequencia (P.S.N)")
                        estado["em_laco"] = True
                        raise Cancelado()
            ...
            # ═══ TODA FERRAMENTA FICA REGISTRADA ══════════════ (18/09) ═══
            #
            # Hoje o Fred pediu "Meus repositorios no GitHub", com a conexao
            # do GitHub ligada. O Bigode leu a pasta `projetos` do disco e
            # listou 34 pastas locais como se fossem repositorios.
            #
            # Para saber POR QUE, a pergunta e: ele chegou a chamar
            # github_repos? Chamou e falhou? Nem tentou? Nenhum arquivo
            # respondia isso -- a tela mostra os passos em linguagem de
            # gente ("Conferindo nos arquivos de verdade") e some quando a
            # conversa e apagada.
            #
            # Este e o mesmo remedio que resolveu o navegador_ver em uma
            # rodada, depois de tres consertos por palpite. Instrumentar
            # primeiro, consertar depois -- e nao o contrario.
            #
            # Passa por aqui TUDO o que nao e navegador, e tambem o que e:
            # esta linha fica depois do if/else de proposito.
            try:
                anotar_erro("[ferramenta] %s  args=%s  %.1fs  ->  %s"
                            % (nome,
                               json.dumps(args, ensure_ascii=False)[:200],
                               duracao,
                               (resultado or "(vazio)").strip()[:300]
                               .replace("\n", " | ")))
            except Exception:
                pass
