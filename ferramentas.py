...
def _desenhar(descricao="", formato="", nome="", modelo="", negativo="",
    ...
    """
    # A PIPI PRIMEIRO, O ComfyUI DEPOIS                        (17/09)
    #
    #   O `imagens.py` fala com o ComfyUI em 127.0.0.1:8188 -- que NAO sobe
    #   nesta maquina: o PyTorch instalado aqui e a compilacao +cpu, que
    #   ignora placa. Entao esta ferramenta existia e apontava para um
    #   servidor que nunca atende.
    #
    #   A Pipi, ao lado, desenha de verdade: FLUX.1-schnell numa L40S na
    #   Modal, com formato, pasta, mosaico, historico e custo por imagem.
    #   Tudo isso ja esta pronto do outro lado -- pedir a ela e melhor do
    #   que o Bigode aprender a desenhar sozinho e as duas versoes
    #   divergirem com o tempo.
    #
    #   O ComfyUI fica como reserva, para quem tiver um de verdade ligado.
    #   18/09: aqui a condicao era `pipi_cliente.disponivel()`, que exige a
    #   pasta E o segredo. Faltando o ponte.txt, esta funcao caia calada no
    #   ComfyUI -- que nao sobe nesta maquina -- e o Fred recebia um erro
    #   sobre modulo de desenho local, sem nenhuma pista de que o problema
    #   era um arquivo de uma linha na pasta ao lado.
    #
    #   O `desenhar` ja sabe explicar cada caso (sem pasta, sem segredo,
    #   Pipi fechada, Modal sem credito) com a frase certa. Entao basta a
    #   PASTA existir para entregar a ele: quem tem a Pipi instalada merece
    #   a mensagem da Pipi, nao a do caminho reserva.
    try:
        import pipi_cliente
        if pipi_cliente._pasta_da_pipi():
            return pipi_cliente.desenhar(descricao, formato=formato,
                                         semente=semente)
    except Exception:
        pass        # a Pipi e um caminho, nao o unico

    try:
        import imagens
    except Exception as erro:
        return ("Nao consegui desenhar. A Pipi IA nao respondeu e o modulo "
                "de desenho local nao carregou (%s).\n\n"
                "O caminho normal e abrir a Pipi: pipi.bat" % erro)
...
CATALOGO = {
    ...
    # AS DUAS PECAS QUE FALTAVAM PARA CONSOLE E TERMINAL       (17/09)
    #
    #   O navegador_ver lista o que e CLICAVEL. Num console de Bash do
    #   PythonAnywhere quase nada e clicavel: o que importa e o texto que
    #   o comando imprimiu -- e isso o ver nao mostra. Sem navegador_texto
    #   o Bigode roda o comando e nao tem como ler a resposta.
    #
    #   E o navegador_escrever digita em CAMPO DE FORMULARIO, por numero.
    #   Terminal na web nao e campo: e um elemento que escuta tecla. Por
    #   isso o navegador_digitar, que manda as teclas para onde o cursor
    #   estiver, sem precisar de numero.
    "navegador_texto":    {"fn": _no_navegador, "escrita": False, "args": "",
                           "desc": "Le o TEXTO visivel da pagina, inclusive "
                                   "saida de console e terminal. Use para ver "
                                   "o que um comando respondeu."},
    "navegador_digitar":  {"fn": _no_navegador, "escrita": True, "args": "texto",
                           "desc": "Digita onde o cursor esta, tecla por tecla. "
                                   "Para console e terminal, que nao sao campo "
                                   "de formulario. Clique no console antes."},
...
PARAMETROS = {
    ...
    "navegador_texto":   {},
    "navegador_digitar": {"texto": "O que digitar onde o cursor esta"},
...
GRUPOS = {
    ...
    "navegador":  ("navegador_ver", "navegador_texto", "navegador_ir",
                   "navegador_clicar", "navegador_escrever",
                   "navegador_digitar", "navegador_teclar",
                   "navegador_rolar", "navegador_esperar"),
...
def ativas():
    ...
    for nome, dados in CATALOGO.items():
        ...
        if nome == "rodar_comando" and not ligada("terminal", False):
            continue
        # Desenho: entra no catalogo se HOUVER quem desenhe.
        #
        # A ideia original continua certa: oferecer uma ferramenta que so
        # pode responder "o desenhista nao esta ligado" gasta tokens do
        # prompt e ensina o modelo a tentar o que nao existe.
        #
        # O QUE ESTAVA ERRADO ERA A PERGUNTA                    (20/09)
        #
        #   O teste era `imagens.no_ar()` -- ou seja, "o ComfyUI esta de
        #   pe?". E o ComfyUI NAO sobe nesta maquina: o PyTorch instalado
        #   aqui e a compilacao +cpu. Por isso a Pipi existe.
        #
        #   Resultado: `gerar_imagem` ficava permanentemente FORA do
        #   catalogo. O Fred pediu "crie uma imagem de um cafe ao
        #   amanhecer" e o Bigode respondeu, com toda a razao do mundo,
        #   que nao sabia criar imagens -- porque ninguem tinha contado a
        #   ele que sabia.
        #
        #   E o atalho de desenho direto que escrevi anteontem testa
        #   `"gerar_imagem" in ativas()` antes de disparar. Ele nunca
        #   disparou. Dois consertos empilhados sobre uma porta fechada
        #   dois andares abaixo.
        #
        #   Agora a pergunta e a certa: existe ALGUEM que desenhe? A Pipi
        #   conta, e e o caminho normal desde 17/09. O ComfyUI continua
        #   valendo para quem tiver um de pe.
        if nome == "gerar_imagem":
            if not ligada("desenho", True):
                continue
            tem_desenhista = False
            try:
                import pipi_cliente
                tem_desenhista = bool(pipi_cliente._pasta_da_pipi())
            except Exception:
                pass
            if not tem_desenhista:
                try:
                    import imagens
                    tem_desenhista = imagens.no_ar()
                except Exception:
                    tem_desenhista = False
            if not tem_desenhista:
                continue
...
# PEDIR LICENCA PARA FAZER O QUE FOI PEDIDO NAO PROTEGE NINGUEM (18/09)
#
#   "crie uma imagem de um cafe ao amanhecer, formato story"
#       -> o Bigode: "Posso continuar?"
#       -> 13,9 segundos esperando
#       -> e a imagem nunca saiu.
#
#   `gerar_imagem` esta marcada com escrita=True porque grava um PNG. Mas a
#   pergunta de autorizacao existe para o que voce NAO pediu: apagar
#   arquivo, rodar comando, publicar. Perguntar "posso?" logo depois de
#   "crie" nao acrescenta seguranca nenhuma -- e gasta a unica coisa que a
#   autorizacao tem de valioso, que e a sua atencao.
#
#   Quem clica "sim" cinco vezes por dia em pedidos obvios clica "sim" no
#   sexto sem ler. O aviso que protege e o que aparece pouco.
#
#   O PNG vai para a pasta producao\ da Pipi, que existe para isso. Nao
#   sobrescreve nada: cada arquivo nasce com nome proprio.
SEM_AUTORIZACAO = {"gerar_imagem"}


def exige_autorizacao(nome):
    if nome in SEM_AUTORIZACAO:
        return False
