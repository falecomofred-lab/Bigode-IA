...
def _e_o_atual(arquivo, atual):
    """Este .gguf e o que esta carregado agora?

    Compara pelo NOME, nao pelo caminho. A mesma IA pode estar em mais de
    uma pasta, e qual delas vence depende da ordem de `pastas()` -- que
    mudou em 18/09. Comparando caminho, uma mudanca de ordem faria o
    modelo carregado aparecer como "LIGAR" em vez de "EM USO".

    Escrito como funcao, e nao como expressao dentro do dicionario, porque
    a versao de uma linha (`a or b if atual else False`) depende de
    precedencia para ser lida certo -- e codigo que precisa de tabela de
    precedencia para ser entendido e codigo que um dia sera lido errado.
    """
    if not atual:
        return False
    if str(arquivo) == atual:
        return True
    try:
        return arquivo.name.lower() == Path(atual).name.lower()
    except Exception:
        return False


def _anotar(texto):
    """Escreve no mesmo erros.log que o cerebro.py usa.

    NAO importa o cerebro.py para reaproveitar o `anotar_erro` de la: o
    cerebro.py importa este arquivo, e o caminho de volta fecharia um
    circulo de importacao -- o Python levantaria ImportError na partida,
    e o Bigode nem abriria. Sao oito linhas duplicadas contra um programa
    que nao sobe; o preco esta claro.

    Falhar aqui nao pode derrubar nada: registro e testemunha, nao motor.
    """
    try:
        with (APP_ROOT / "erros.log").open("a", encoding="utf-8") as f:
            f.write("\n" + "=" * 70 + "\n%s  %s\n"
...
#
# ══ "Pen IA" NAO E UM PENDRIVE ═══════════════════════════════ (18/09) ══
#
#   O nome enganava, inclusive a mim. `G:\Outros computadores\USB e
#   dispositivos externos\Pen IA` e uma PASTA DO GOOGLE DRIVE -- a copia de
#   um pendrive antigo, que o Drive espelha. Nao ha nada plugado.
#
#   O comentario que estava aqui dizia:
#
#       "O pendrive vem primeiro. Quando ele esta plugado, ler dali e
#        instantaneo; o MESMO arquivo pelo G: e um download do Drive."
#
#   As duas metades da frase apontam para o mesmo G:. E pior: a ordem que
#   ela prometia NAO EXISTIA no codigo -- o `pastas()` punha RAIZ e
#   APP_ROOT na frente. Resultado pratico de hoje: o DeepSeek existe nas
#   duas pastas, o `listar()` deduplica por nome, e venceu a copia de
#   `Meu Drive\projetos\Cerebro` -- justamente a que o Fred nunca escolheu.
#
#   Agora a pasta de modelos vem PRIMEIRO, de verdade, e e uma so.
MODELOS = r"G:\Outros computadores\USB e dispositivos externos\Pen IA"
MODELOS_COLAB = "/content/drive/Othercomputers/USB e dispositivos externos/Pen IA"

# Nomes antigos, para nao quebrar quem importa daqui.
PEN_WINDOWS = MODELOS
PEN_COLAB = MODELOS_COLAB

PASTAS_PADRAO = [MODELOS, MODELOS_COLAB, "/content/modelos"]

# AS IAs DE IMAGEM FICAM DE FORA DESTA LISTA, DE PROPOSITO
#
#     Elas moram em `Pen IA\IA Imagem` (flux1-dev, qwen-image-edit) e quem
#     le e o ComfyUI, nao o llama.cpp. Se entrassem aqui, apareceriam na
#     tela junto com as de texto e o Bigode tentaria carregar um modelo de
#     difusao como se fosse de conversa -- falha com uma mensagem que nao
#     explica nada.
#
#     O que as mantem de fora e o `glob("*.gguf")` do `listar()`, que NAO
#     desce para subpastas. Por isso `IA Imagem` e `ComfyUI\models\unet`
#     ficam invisiveis aqui sem precisar de lista de excecao.
#
...
class LocalModelProvider:
    ...
    def pastas(self):
        ...
        A ORDEM DECIDE QUAL COPIA VENCE                       (18/09)

            `listar()` deduplica por NOME de arquivo: a primeira pasta que
            tiver `X.gguf` e a que vale. Entao esta ordem nao e detalhe --
            e ela que diz de onde o modelo vai ser lido.

            Estava assim:

                [RAIZ, APP_ROOT] + extras + PASTAS_PADRAO

            RAIZ e `G:\\Meu Drive\\projetos`; APP_ROOT e a pasta do Cerebro.
            As duas na frente da pasta de modelos. O DeepSeek existe nas
            duas, e venceu a copia solta dentro do Cerebro -- nao a da
            pasta oficial. O Fred clicou no mesmo botao e carregou um
            arquivo que nem sabia que estava ali.

            Agora a pasta de modelos vem primeiro, com as configuradas a
            mao logo atras. RAIZ e APP_ROOT ficam por ultimo, como rede de
            seguranca para quem largou um .gguf ao lado do programa -- nao
            mais como primeira escolha.
        """
        extras = [Path(p) for p in (self._config().get("pastas_modelos") or [])]
        todas = ([Path(p) for p in PASTAS_PADRAO] + extras + [RAIZ, APP_ROOT])
    ...
    def listar(self):
        ...
        for pasta in self.pastas():
            ...
            for arquivo in sorted(pasta.glob("*.gguf")):
                ...
                #
                # Vence a PRIMEIRA pasta da lista. Desde 18/09 essa e a
                # pasta oficial de modelos (`Pen IA`), e nao mais a pasta do
                # programa -- ver a nota em `pastas()`.
                ...
                saida.append({
                    ...
                    "incompleto": gb < 1.0,
                    # COMPARA PELO NOME, NAO PELO CAMINHO           (18/09)
                    #
                    #   Era `str(arquivo) == atual`. Hoje, ao pôr a pasta
                    #   oficial na frente, o caminho vencedor do DeepSeek
                    #   mudou de `...\\Cerebro\\X.gguf` para
                    #   `...\\Pen IA\\X.gguf` -- e o `modelo_atual` gravado
                    #   no config continua apontando para o antigo.
                    #
                    #   Com a comparacao por caminho, o modelo carregado
                    #   apareceria como "LIGAR" em vez de "EM USO", e o
                    #   `atual()` devolveria None -- o que faz o ajuste de
                    #   janela e o aquecimento perderem a referencia.
                    #
                    #   O mesmo nome de arquivo e o mesmo modelo, venha da
                    #   pasta que vier: e exatamente o criterio que o
                    #   `vistos` logo acima ja usa para deduplicar.
                    "atual": _e_o_atual(arquivo, atual),
    ...
    def remoto(self):
        """O motor mora fora desta maquina?

        MOTOR REMOTO NAO TEM CICLO DE VIDA AQUI               (17/09)
            Com o provedor em Modal ou Hugging Face, nao existe processo
            local para subir, derrubar ou vigiar. Quem liga e desliga a
            placa e a Modal, sozinha, quando chega e para de chegar pedido.
        """
        try:
            return str(self._config().get("modelo_provedor") or
                       "local").lower() in ("modal", "huggingface")
        except Exception:
            return False
    ...
    def online(self):
        ...
        ISTO DIZIA "DESLIGADO" COM O MOTOR FUNCIONANDO        (17/09)
            A linha antiga era:

                base = "http://%s:%d/" % (hostname_do_llm_url, self._porta())

            Ela pegava o HOST do llm_url e colava a PORTA LOCAL. Com o
            provedor em Modal, o llm_url e

                https://...-bigode-motor-servidor.modal.run/v1/chat/completions

            e o resultado era

                http://...modal.run:8082/

            Endereco que nao existe: porta errada e sem TLS. Falhava
            sempre, a tela dizia "motor desligado", e o Bigode subia o
            llamafile local para "consertar" -- gastando 6,3 GB de RAM e
            doze threads com um motor que nao ia ser usado.

            O Fred viu tres "novo carregamento" no log tendo a Modal
            configurada e respondendo.
        """
        cfg = self._config()
        bruto = str(cfg.get("llm_url") or "")

        if self.remoto():
            # Preferir o health_url quando existe: no llama-server da Modal
            # ele responde 200 pronto e 503 carregando, o que e informacao
            # melhor do que "abriu a conexao".
            alvo = str((cfg.get("modal") or {}).get("health_url") or "").strip()
            if not alvo:
                endereco = urlparse(bruto)
                if not endereco.scheme:
                    return False
                alvo = "%s://%s/health" % (endereco.scheme, endereco.netloc)
            try:
                with urllib.request.urlopen(alvo, timeout=8) as r:
                    return 200 <= r.status < 300
            except urllib.error.HTTPError as e:
                # 503 = llama.cpp carregando o modelo. Ainda nao esta
                # pronto, mas o servico existe -- e 401/403 significa que
                # ele esta de pe e so recusou a chamada sem token.
                return e.code in (401, 403)
            except Exception:
                return False

        # ═══ 503 NAO E "PRONTO" ══════════════════════════════ (18/09) ═══
        #
        #   Aqui estava:
        #
        #       except urllib.error.HTTPError:
        #           return True          # qualquer resposta HTTP = de pe
        #
        #   O llama-server responde 503 "Loading model" ENQUANTO carrega.
        #   Com a regra acima, isso contava como pronto.
        #
        #   O estrago: o Fred ligou o DeepSeek (10,4 GB, lido do Google
        #   Drive). Durante os minutos de carga, `online()` dizia True, o
        #   Bigode mandava a pergunta assim mesmo, o motor recusava, e a
        #   tela escolhia a mensagem do `else`:
        #
        #       "O motor derrubou a conexao."
        #
        #   O motor nao derrubou nada -- ele estava dizendo, com todas as
        #   letras e no codigo HTTP certo, que ainda estava carregando. O
        #   cerebro.py tem a mensagem certa para esse caso, e ela nunca era
        #   escolhida porque esta funcao mentia sobre o estado.
        #
        #   O caminho remoto ja tratava 503 direito desde ontem. O local
        #   ficou para tras -- os dois fazem a mesma pergunta e deviam ter
        #   a mesma resposta.
        endereco = urlparse(bruto)
        base = "http://%s:%d/health" % (endereco.hostname or "127.0.0.1",
                                        self._porta())
        try:
            with urllib.request.urlopen(base, timeout=4) as r:
                return 200 <= r.status < 300
        except urllib.error.HTTPError as e:
            # 503 = carregando: existe, mas nao atende. 401/403 = de pe e
            # so recusou sem chave. 404 = versao antiga sem /health, e
            # responder ja prova que esta no ar.
            if e.code == 503:
                return False
    ...
    def carregar(self, caminho):
        # NAO SUBIR MOTOR LOCAL COM PROVEDOR REMOTO            (17/09)
        #
        #   Este e o unico ponto onde um processo de motor nasce. Com o
        #   provedor em Modal ou Hugging Face, subir o llamafile aqui
        #   significa 6,3 GB de RAM e doze threads ocupados por um modelo
        #   que nao vai atender pedido nenhum -- e a maquina fica lenta
        #   justamente por causa do motor que foi posto na nuvem para
        #   deixar de pesar.
        #
        #   Aconteceu: o log do Fred mostrou tres "novo carregamento" do
        #   Qwen 7B local com a Modal ja configurada e respondendo.
        #
        #   A tranca fica aqui, e nao em quem chama, porque sao varios os
        #   caminhos que chegam neste metodo: o botao LIGAR da tela, o
        #   ajuste de janela, o aquecimento da partida.
        if self.remoto():
            return {"ok": False,
                    "msg": ("o motor esta na nuvem (%s). Para voltar ao "
                            "modelo local, rode `python usar_modal_motor.py "
                            "--remover` ou `python usar_huggingface.py "
                            "--remover`."
                            % str(self._config().get("modelo_provedor")))}
        ...
        # ═══ QUEM MANDOU SUBIR ESTE MODELO? ══════════════════ (18/09) ═══
        #
        #   O Fred ligou o DeepSeek e o log provou que subiu. Depois de um
        #   reinicio, voltou o Qwen 7B -- e nao ha como saber por que: este
        #   metodo e chamado de cinco lugares (botao LIGAR, partida,
        #   `garantir_carregado`, ajuste de janela, aquecimento) e nenhum
        #   deles deixa rastro.
        #
        #   Nao vou adivinhar de novo. Cada carga passa a registrar o nome
        #   do modelo e a PILHA de quem pediu -- na proxima vez que trocar
        #   sozinho, o log diz o culpado em vez de eu propor a quarta
        #   hipotese.
        try:
            import traceback
            quem = [l.strip() for l in traceback.format_stack()[:-1]
                    if "cerebro.py" in l or "modelos.py" in l]
            _anotar("[carregar] %s\n    pedido por: %s"
                    % (modelo.name, " <- ".join(reversed(quem[-4:]))[:400]))
        except Exception:
            pass
